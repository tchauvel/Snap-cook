#!/bin/bash
# Script to create a standalone development build that doesn't require Metro
...
# Check if development client is installed
if ! grep -q "expo-dev-client" package.json; then
  echo "Installing expo-dev-client..."
  npx expo install expo-dev-client
fi

# Add debug alerts to ingredient scanner
echo "Adding debug alerts to ingredient scanner..."
cat > debug-fix.js << 'EOF'
const fs = require('fs');
...
echo ""
echo "This will create a build that you can install directly on your device"
echo "without requiring a connection to Metro bundler."
echo ""

# Choose the platform
echo "Which platform would you like to build for?"
echo "1. iOS (requires Mac and Xcode)"
echo "2. Android (requires Android SDK)"
...
case $PLATFORM_CHOICE in
  1)
    ...
    cd ios
    xcodebuild -workspace snap-and-cook.xcworkspace -scheme snap-and-cook -configuration Debug -destination 'generic/platform=iOS' clean build
    echo "iOS build completed. Look for the .app file in the build/Debug-iphoneos directory."
    ;;
  2)
    ...
    echo "Android build completed. Look for the .apk file in the app/build/outputs/apk/debug directory."
    ;;
  3)
    echo "Building for both platforms..."
    npx expo prebuild
    # iOS build
    (cd ios && xcodebuild -workspace snap-and-cook.xcworkspace -scheme snap-and-cook -configuration Debug -destination 'generic/platform=iOS' clean build)
    # Android build
...
echo "You can now install the build directly on your device without needing Metro." 